...
function AccommodationReview(props) {
  ...
  const findReviews = async (id) => {
    const res = await axios.get(
      `https://elites-barefoot-nomad.herokuapp.com/api/v1/accommodations/${id}/reviews`,
      {
        headers: {
  ...
  return (
    <div>
      <AccommodationViewReviewModal
        reviews={reviews ?? []}
        avg={avgRev}
        reviewsCount={countRatings}
        isOpen={openV}
        handleClose={() => setOpenV(false)}
      ...
      >
        <Grid container spacing={1}>
          {props.accommodationsData.hasOwnProperty('data')
            ? props.accommodationsData.data.payload.map((accommodation) => {
                return (
                  <Grid
