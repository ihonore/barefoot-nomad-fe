...
const { SET_STEP_ONE_REQUESTS,
  ...
  INITIALIZE,
  UPDATE_TRIP_REQUEST } = actionTypes;
...
it ("should set step one requests",()=>{
  ...
  expect(reducer).toEqual({stepOne: {
    requests: 'payload',
    departDate: new Date().toISOString().split('T')[0],
    returnDate:  new Date().toISOString().split('T')[0],
  },
...
it ("should set step two trip reason",()=>{
  const reducer = tripRequestReducer(initialState, setTripRequestReasonActions);
  expect(reducer).toEqual({stepOne: {
    requests: [{ destination: '', departLocation: '', accomodation: '' }],
    departDate: new Date().toISOString().split('T')[0],
    returnDate:  new Date().toISOString().split('T')[0],
...
it ("should add multicity",()=>{
  ...
  expect(reducer).toEqual({stepOne: {
    requests:[{ destination: '', departLocation: '', accomodation: '' },'payload'],
    departDate: new Date().toISOString().split('T')[0],
    returnDate:  new Date().toISOString().split('T')[0],
  },
  stepTwo: { tripReason: '', rememberMe: '' },})
